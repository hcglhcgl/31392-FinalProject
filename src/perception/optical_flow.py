...
class OpticalFlow:
    def __init__(self, points=400):
        self.orb = cv2.ORB_create(points)

    def __get_features(self, frame):
        kp, descs = self.orb.detectAndCompute(frame, None)
        return np.array([np.array([k.pt]).astype(np.float32, ) for k in kp])

    def optical_flow_ORB(self, first: np.ndarray, next: np.ndarray, p0=None):
        ...
        lk_params = dict(winSize=(9, 9),
                         maxLevel=2,
                         criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 30, 0.4))

        # Take first frame and find corners in it
        old_gray = cv2.cvtColor(first, cv2.COLOR_BGR2GRAY)
        if p0 is None:
            p0 = self.__get_features(old_gray)
    ...
    def optical_flow_frames_corner(self, first: np.ndarray, next: np.ndarray, p0=None):
